...
fn hook_run_is_quiet_success(run: &HookRunSummary) -> bool {
    run.status == HookRunStatus::Completed
        && (run.entries.is_empty() || run.entries.iter().all(is_evomate_dynamic_yes_entry))
}

fn is_evomate_dynamic_yes_entry(entry: &HookOutputEntry) -> bool {
    entry.kind == HookOutputEntryKind::Context
        && entry
            .text
            .trim_start()
            .starts_with("╭─ EvoMate Dynamic Yes")
}
