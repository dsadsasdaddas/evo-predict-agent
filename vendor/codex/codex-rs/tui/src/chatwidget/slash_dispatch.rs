...
const EVOMATE_SLASH_TIMEOUT: Duration = Duration::from_millis(2500);
const EVOMATE_PROGRESS_SETTLE: Duration = Duration::from_millis(90);
...
impl ChatWidget {
    ...
    fn handle_evomate_train_command(&mut self, args: &str) {
        let args = args.trim().to_string();
        let cwd = self.config.cwd.display().to_string();
        let tx = self.app_event_tx.clone();
        let show_progress = !self.bottom_pane.is_task_running();
        if show_progress {
            self.on_evomate_slash_progress(
                "EvoMate training".to_string(),
                "init slash request".to_string(),
                8,
            );
        } else {
            self.add_info_message(
                "EvoMate /train → queued sidecar request while Codex is busy…".to_string(),
                Some("HUD train node updates when EvoMate writes a receipt.".to_string()),
            );
        }

        let spawn_result = std::thread::Builder::new()
            .name("evomate-slash-train".to_string())
            .spawn(move || {
                let (message, hint, is_error) =
                    match evomate_train_message(&args, &cwd, show_progress.then_some(tx.clone())) {
                        Ok((message, hint)) => (message, hint, false),
                        Err(err) => (format!("EvoMate /train failed: {err}"), None, true),
                    };
                tx.send(AppEvent::EvoMateSlashResult {
                    message,
                    hint,
                    is_error,
                });
        ...
        if let Err(err) = spawn_result {
            self.add_error_message(format!("EvoMate /train failed to start worker: {err}"));
        }
    }

    pub(crate) fn on_evomate_slash_progress(&mut self, header: String, stage: String, percent: u8) {
        if self.bottom_pane.is_task_running() && !self.evomate_slash_progress_active {
            return;
        }
        self.evomate_slash_progress_active = true;
        self.bottom_pane.ensure_status_indicator();
        self.bottom_pane
            .set_interrupt_hint_visible(/*visible*/ false);
        self.set_status(
            header,
            Some(evomate_progress_line(percent, &stage)),
            StatusDetailsCapitalization::Preserve,
            /*details_max_lines*/ 1,
        );
    }

    pub(crate) fn on_evomate_slash_result(
        &mut self,
        message: String,
        hint: Option<String>,
        is_error: bool,
    ) {
        if self.evomate_slash_progress_active {
            self.evomate_slash_progress_active = false;
    ...
    fn handle_evomate_history_command(&mut self, args: &str) {
        let args = args.trim().to_string();
        let tx = self.app_event_tx.clone();
        self.add_info_message(
            "EvoMate /history → searching evolution memory…".to_string(),
            Some("Usage: /history mcp limit=5 jobs".to_string()),
        );

        let spawn_result = std::thread::Builder::new()
            .name("evomate-slash-history".to_string())
            .spawn(move || {
                let (message, hint, is_error) = match evomate_history_message(&args) {
                    Ok((message, hint)) => (message, hint, false),
                    Err(err) => (format!("EvoMate /history failed: {err}"), None, true),
                };
                tx.send(AppEvent::EvoMateSlashResult {
                    message,
                    hint,
        ...
        if let Err(err) = spawn_result {
            self.add_error_message(format!("EvoMate /history failed to start worker: {err}"));
        }
    }
    ...
    pub(super) fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Train => {
                self.handle_evomate_train_command("");
            }
            SlashCommand::History => {
                self.handle_evomate_history_command("");
            }
            SlashCommand::Ide => {
    ...
    fn dispatch_prepared_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Train => {
                self.handle_evomate_train_command(trimmed);
            }
            SlashCommand::History => {
                self.handle_evomate_history_command(trimmed);
            }
...
fn evomate_api_base() -> String {
    std::env::var("EVOMATE_API_URL")
        .unwrap_or_else(|_| "http://127.0.0.1:8787".to_string())
        .trim_end_matches('/')
        .to_string()
}

fn evomate_train_message(
    args: &str,
    cwd: &str,
    progress_tx: Option<AppEventSender>,
) -> Result<(String, Option<String>), String> {
    send_evomate_train_progress(progress_tx.as_ref(), 18, "capture local evolution dataset");
    let train_type = parse_evomate_train_type(args);
    let execute_remote = args
        .split_whitespace()
        .any(|token| matches!(token.to_ascii_lowercase().as_str(), "remote" | "--remote"));
    send_evomate_train_progress(progress_tx.as_ref(), 34, "select training objective");
    let payload = json!({
        "type": train_type,
        "source": "codex_slash_train",
        "executeRemote": execute_remote,
        "metadata": {
            "slash": "/train",
            "args": args,
            "cwd": cwd
    ...
    send_evomate_train_progress(progress_tx.as_ref(), 58, "queue job through EvoMate API");
    let json = evomate_post_json("/api/evolution/train", payload)?;
    send_evomate_train_progress(progress_tx.as_ref(), 84, "write GEP receipt + HUD pulse");
    let job = json.get("job").unwrap_or(&Value::Null);
    let job_id = job
        .get("jobId")
    ...
    let job_type = job
        .get("type")
        .and_then(Value::as_str)
        .unwrap_or(train_type);
    send_evomate_train_progress(progress_tx.as_ref(), 100, "queued + receipt committed");
    Ok((
        format!(
            "EvoMate /train {}\nqueued {job_type} · {status} · {mode}\njob: {job_id}",
            evomate_progress_line(100, "queued + receipt")
        ),
        Some(
            "HUD train node updates from remote_job_queued; use /history jobs to inspect."
                .to_string(),
        ),
    ))
}

fn send_evomate_train_progress(tx: Option<&AppEventSender>, percent: u8, stage: &str) {
    let Some(tx) = tx else {
        return;
    };
    tx.send(AppEvent::EvoMateSlashProgress {
        header: "EvoMate training".to_string(),
        stage: stage.to_string(),
        percent,
    });
    std::thread::sleep(EVOMATE_PROGRESS_SETTLE);
}

fn evomate_progress_line(percent: u8, stage: &str) -> String {
    let percent = percent.min(100);
    let cells = 10usize;
    let filled = ((usize::from(percent) * cells) + 99) / 100;
...
fn evomate_history_message(args: &str) -> Result<(String, Option<String>), String> {
    let json = evomate_get_json(&evomate_history_path(args))?;
    let count = json.get("count").and_then(Value::as_u64).unwrap_or(0);
    let total = json
        .get("totalTimeline")
        .and_then(Value::as_u64)
        .unwrap_or(count);
    let mut lines = vec![format!("EvoMate /history · {count}/{total} event(s)")];
    if let Some(items) = json.get("timeline").and_then(Value::as_array) {
        for item in items.iter().take(6) {
            let event = item.get("type").and_then(Value::as_str).unwrap_or("event");
            let score = item
                .get("score")
                .and_then(Value::as_f64)
                .map(|value| format!("{}%", (value.clamp(0.0, 1.0) * 100.0).round()))
                .unwrap_or_else(|| "--".to_string());
            let gene = item.get("geneId").and_then(Value::as_str).unwrap_or("-");
            let summary = item
                .get("summary")
                .and_then(Value::as_str)
                .map(|value| compact_evomate_text(value, 72))
                .unwrap_or_default();
            lines.push(format!("• {event} · {score} · {gene} · {summary}"));
        }
    }
    if let Some(jobs) = json.get("jobs").and_then(Value::as_array) {
        if !jobs.is_empty() {
            lines.push("jobs:".to_string());
            for job in jobs.iter().take(4) {
                let job_id = job.get("jobId").and_then(Value::as_str).unwrap_or("job");
                let job_type = job.get("type").and_then(Value::as_str).unwrap_or("-");
                let status = job.get("status").and_then(Value::as_str).unwrap_or("-");
                lines.push(format!("• {job_id} · {job_type} · {status}"));
            }
        }
    }
    Ok((
...
fn evomate_client() -> Result<reqwest::blocking::Client, String> {
    reqwest::blocking::Client::builder()
        .timeout(EVOMATE_SLASH_TIMEOUT)
        .connect_timeout(EVOMATE_SLASH_TIMEOUT)
        .no_proxy()
        .build()
        .map_err(|err| err.to_string())
}

fn evomate_post_json(path: &str, payload: Value) -> Result<Value, String> {
    let client = evomate_client()?;
    let response = client
        .post(format!("{}{}", evomate_api_base(), path))
        .json(&payload)
        .send()
        .map_err(|err| err.to_string())?;
    decode_evomate_response(response)
}

fn evomate_get_json(path: &str) -> Result<Value, String> {
    let client = evomate_client()?;
    let response = client
        .get(format!("{}{}", evomate_api_base(), path))
        .send()
        .map_err(|err| err.to_string())?;
    decode_evomate_response(response)
}

fn decode_evomate_response(response: reqwest::blocking::Response) -> Result<Value, String> {
    let status = response.status();
    let text = response.text().map_err(|err| err.to_string())?;
    let json = serde_json::from_str::<Value>(&text)
        .map_err(|err| format!("invalid json from EvoMate API: {err}; body={text}"))?;
...
fn parse_evomate_train_type(args: &str) -> &'static str {
    let lower = args.to_ascii_lowercase();
    if lower.contains("embedding") || lower.contains("memory") || lower.contains("retrieval") {
        "embedding_build"
    } else if lower.contains("policy") || lower.contains("replay") {
        "policy_replay_eval"
    } else if lower.contains("gym") || lower.contains("eval") {
        "evolution_gym_eval"
    } else {
        "preference_train"
    }
...
fn evomate_history_path(args: &str) -> String {
    let mut limit = "6".to_string();
    ...
    for token in args.split_whitespace() {
        if token.eq_ignore_ascii_case("jobs") || token == "--jobs" {
            jobs = true;
            continue;
        }
        ...
        if let Some(value) = token.strip_prefix("gene=") {
            gene_id = Some(value.to_string());
            continue;
        }
        if let Some(value) = token.strip_prefix("geneId=") {
            gene_id = Some(value.to_string());
            continue;
        }
        query.push(token);
    ...
    let mut params = vec![("limit", limit), ("jobs", jobs.to_string())];
    if !query.is_empty() {
        params.push(("q", query.join(" ")));
    }
    if let Some(value) = event_type {
        params.push(("type", value));
    }
    if let Some(value) = gene_id {
        params.push(("geneId", value));
    }
    let query = params
        .into_iter()
        .map(|(key, value)| {
        ...
        .collect::<Vec<_>>()
        .join("&");
    format!("/api/evolution/history?{query}")
}
...
fn compact_evomate_text(value: &str, max_chars: usize) -> String {
