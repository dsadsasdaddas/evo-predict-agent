...
use crate::evomate_status::EvoMateStatusWidget;
...
pub(crate) struct App {
    ...
    evomate_status: EvoMateStatusWidget,
...
See the Codex keymap documentation for supported actions and examples."
        ...
        let mut app = Self {
            ...
            app_event_tx,
            chat_widget,
            evomate_status: EvoMateStatusWidget::new(tui.frame_requester()),
        ...
        if self.overlay.is_some() {
            ...
            match event {
                ...
                TuiEvent::Draw | TuiEvent::Resize => {
                    ...
                    self.evomate_status.pre_draw_tick();
                    let rendered_area =
    ...
    fn render_chat_widget_frame(
        ...
        let evomate_height = self.evomate_status.desired_height(terminal_width);
        let desired_height = self
            .chat_widget
            .desired_height(terminal_width)
            .saturating_add(evomate_height);
        let mut rendered_area = Rect::default();
        if terminal_resize_reflow_enabled {
            tui.draw_with_resize_reflow(desired_height, |frame| {
                let area = frame.area();
                rendered_area = area;
                let (evomate_area, chat_area) = split_evomate_chat_area(area, evomate_height);
                self.evomate_status.render(evomate_area, frame.buffer);
...
fn split_evomate_chat_area(area: Rect, desired_evomate_height: u16) -> (Rect, Rect) {
    let evomate_height = desired_evomate_height.min(area.height.saturating_sub(1));
    let evomate_area = Rect::new(area.x, area.y, area.width, evomate_height);
    let chat_area = Rect::new(
        area.x,
        area.y.saturating_add(evomate_height),
        area.width,
        area.height.saturating_sub(evomate_height),
    );
    (evomate_area, chat_area)
}
